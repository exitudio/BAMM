...
def evaluation_res_transformer(out_dir, val_loader, trans, vq_model, writer, ep, best_fid, best_div,
    ...
    for batch in val_loader:
        ...
        if ep == 0:
            ...
            mids = t2m_transformer.generate(clip_text, m_length // 4, timesteps=3, cond_scale=4,
...
def evaluation_mask_transformer_test_plus_res(val_loader, vq_model, res_model, trans, repeat_id, eval_wrapper,
    ...
    for i, batch in enumerate(tqdm(val_loader)):
        ...
        # (b, seqlen, c)
            ...
            for _ in range(30):
                ...
                # pred_codes = trans(code_indices[..., 0], clip_text, input_len//4, force_mask=force_mask)
        ...
        et, em = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pose, input_len)
        motion_annotation_list.append(em)
        motion_pred_list.append(em_pred)
    ...
    msg = f"--> \t Eva. Repeat {repeat_id} :, FID. {fid:.4f}, " \
          ...
          f"multimodality. {multimodality:.4f}"
    print(msg)
    return fid, diversity, R_precision, matching_score_pred, multimodality, msg