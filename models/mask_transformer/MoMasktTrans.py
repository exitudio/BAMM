...
class MaskTransformer(nn.Module):
    ...
    def generate(self,
        ...
        _is_mask = torch.ones_like(ids, dtype=torch.bool)
        _is_mask[:, 1::2] = False
        ids[_is_mask] = self.mask_id
        ids = torch.where(padding_mask, self.pad_id, ids)
        scores = torch.zeros_like(padding_mask, dtype=torch.double)
        scores[~_is_mask] = 9e4
        scores = torch.where(padding_mask, 1e5, scores)
